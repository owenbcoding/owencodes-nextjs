...
function buildSingleEntryPayload(entry) {
  ...
  const verbNoun = isNew ? "New archive" : "Updated archive";
  const verbPhrase = isNew ? "published a new archive" : "updated an archive";
...
function buildDigestPayload(entries, siteUrl) {
  ...
  const summaryParts = [];
  if (newCount) summaryParts.push(`${newCount} new archive${newCount > 1 ? "s" : ""}`);
  if (updatedCount) summaryParts.push(`${updatedCount} updated archive${updatedCount > 1 ? "s" : ""}`);
  ...
  return {
    subject: `Newsletter updates: ${summaryParts.join(" and ")}`,
...
async function main() {
  ...
  const raw = process.env.CHANGED_NEWSLETTER_FILES || "";
  const changes = parseChangedFiles(raw);

  if (changes.length === 0) {
    console.log("No relevant newsletter changes to notify about.");
    return;
  }
  ...
  if (entries.length === 0) {
    console.log("No newsletter entries could be parsed; skipping notification.");
    return;
  }
